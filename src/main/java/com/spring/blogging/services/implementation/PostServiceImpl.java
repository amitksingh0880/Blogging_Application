...
public class PostServiceImpl implements PostService {
    ...
    @Override
    public List<PostDto> getPostByCategory(Integer categoryId) {  //TO Implement Pagination
...
////        int pageSize = 5;                                            //
////        int pageNumber = 1;                                          //
//        Pageable pageable = PageRequest.of(pageNumber,pageSize);     // ===== Paging
...
//    }
    public PostResponse getAllPost(Integer pageNumber , Integer pageSize , String sortBy,String order) {
        ...
        Pageable pageable = PageRequest.of(pageNumber,pageSize , sort);     // ===== Paging
        Page<Post> pagePosts = this.postRepo.findAll(pageable);      //
        List<Post> allPosts = pagePosts.getContent();                //
        List<PostDto> postDtos = allPosts.stream().map((post)-> this.modelMapper.map(post, PostDto.class)).collect(Collectors.toList());
    ...
    @Override
    public List<PostDto> getPostByUser(Integer userId) { //TO Implement Pagination
        User user = this.userRepo.findById(userId).orElseThrow(()-> new ResourceNotFoundException("User","User id",userId));
